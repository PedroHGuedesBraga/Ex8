...
public class PesquisaFilmeAno implements ActionListener {
    private SistemaFilmeGUI screen;
    private SistemaFilmeInterface sistemaFilmeMap;

    public PesquisaFilmeAno(SistemaFilmeGUI sistemaGUI, SistemaFilmeMap sistemaFilmeMap) {
        this.screen = sistemaGUI;
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if(filmes.isEmpty()){
            JOptionPane.showMessageDialog(null,"não foi encontrado, nenhum filme cadastrado, que lançado no ano de "+ano);
        }else {
            int quant = 0;
            for (Filme f : filmes) {
                quant++;
